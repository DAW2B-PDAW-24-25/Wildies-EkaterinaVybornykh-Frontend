...
function Inicio() {
    ...
    async function cargarUsuario() {             // TODO pasar al formulario de login
        try {
            let response = await fetch(`${API_URL}/api/usuarios/1`);
            if (!response.ok) {
    ...
    async function cargarUsuarios() {
        try {
            let response = await fetch(`${API_URL}/api/usuarios/usuariosCerca/1}`);        //todo cambiar 1 por usuarioLogeado.id
    ...
    async function cargarEventos() {
        try {
            let response = await fetch(`${API_URL}/api/eventos/eventosCerca/1`);     //todo cambiar 1 por usuarioLogeado.id
